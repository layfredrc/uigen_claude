...
function FileTreeNode({ node, level }: FileTreeNodeProps) {
  ...
  return (
    <div>
      <div
        className={cn(
          "flex items-center gap-2 px-2 py-1.5 hover:bg-gray-100 dark:hover:bg-white/[0.05] cursor-pointer text-sm transition-colors",
          selectedFile === node.path && "bg-blue-50 dark:bg-violet-500/10 text-blue-600 dark:text-violet-400"
        )}
      ...
      >
        {node.type === "directory" ? (
          <>
            ...
            {isExpanded ? (
              <FolderOpen className="h-4 w-4 shrink-0 text-blue-500 dark:text-violet-400" />
            ) : (
              <Folder className="h-4 w-4 shrink-0 text-blue-500 dark:text-violet-400" />
            )}
          </>
        ) : (
          <>
  ...
  if (!rootNode || !rootNode.children || rootNode.children.size === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-4 text-center">
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Files will appear here</p>
      </div>
    );
  }
